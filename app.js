...
function requestAuthDeezer() {
    ...
    //let url = AUTHORIZE_DEEZER;
    //url += "?app_id=" + deezer_app_id;
    //url += "&response_type=token";
    //url += "&redirect_uri=" + encodeURI(redirect_uri);
    url += "&show_dialog=true";
    url += "&perms=basic_access,manage_library";
    window.location.href = url;
}


function deezer_callAPI(method, url, body, callback) {
    accessToken = localStorage.getItem("access_token");
...
//1
function deezer_handleRedirect(qstring) {
    const urlparam = new URLSearchParams(qstring);
    var deezer_code = urlparam.get('code');
    //var result = qstring.split('&').reduce(function (res, item) {
    //    var parts = item.split('=');
    //    res[parts[0]] = parts[1];
    //    return res;
    //}, {});
    alert(deezer_code)
    deezer_fetchAccessToken(deezer_code);
    window.history.pushState("", "", redirect_uri);
}

//2
async function deezer_fetchAccessToken(code) {
    await callAuthorizationApiDeezer(code);
}

//3
function callAuthorizationApiDeezer(deezer_code) {

    let url = DEEZER_TOKEN;
    url += "?app_id=" + deezer_app_id;
    url += "&output=json";
    url += "&secret=" + deezer_secret_key;
    url += "&code=" + deezer_code;
    //deezer_callAPI('GET', url, null, handleDeezerAuthResponse())
    window.location.href = url;

    //let url = new URL(DEEZER_TOKEN);
    //url.searchParams.set('app_id', deezer_app_id);
    //url.searchParams.set('secret', deezer_secret_key);
    //url.searchParams.set('code', deezer_code);
    //url.searchParams.set('output', 'json');
    //window.location.href = url;
    //    handleDeezerAuthResponse();
}

//4
//function handleDeezerAuthResponse() {
//    debugger
//    if (this.status == 200) {
//        var data = JSON.parse(this.responseText);
//        console.log(data);
...
//5
function deezer_getUserId() {
    body = null;
    deezer_callAPI('GET', DEEZER_PROFILE, body, handleDeezerProfileRes);
}

3
function handleDeezerProfileRes() {
    debugger
    if (this.status == 200) {
        var data = JSON.parse(this.responseText);
        userId = data.id;
        localStorage.setItem("user_id", userId);
        deezer_createPlaylist();
    }
    else {
        console.log(this.responseText);
        alert(this.responseText);
...
4
function deezer_createPlaylist() {
    if (playlistTitle == "") { playlistTitle = "TITULO PADRAO DEEZER"; }
    let url = DEEZER_CREATE_PLAYLIST + userId + "/playlists";
    let body = JSON.stringify({ "title": playlistTitle + "" });
    deezer_callAPI('POST', url, body, handleDeezerCreatePlaylistRes);
}

//function handleDeezerCreatePlaylistRes() {
//    if (this.status == 201) {
//        var data = JSON.parse(this.responseText);
//        playlistId = data.id;
//        localStorage.setItem("playlist_id", playlistId);
...
//5
//function deezer_searchSongs() {
//    songList = JSON.parse(localStorage.getItem("songList"));
//    localStorage.removeItem("songList");
    //Get all music deezer Ids
//    for (i in songList) {
//        let url = DEEZER_SEARCH + "?q=";
//        url += songList[i];
//        url += "&limit=1";
//        debugger;
//        setTimeout(deezer_callAPI('GET', url, null, handleDeezerSearchRes), 100);
//    }
//}

//6
//function handleDeezerSearchRes() {
//    if (this.status == 200) {
//        var data = JSON.parse(this.responseText);
//        //songId = data.data.id;
//        if (data.total == 0) { alert("Music not found."); not_found++; }
//        else {
//            songId += data.items[0].id.toString();
//            songsIds.push(songId);
//       }
//        if (songList.length == songsIds.length + not_found) { deezer_addSongs(songsIds); }
//    }
//    else {
//        console.log(this.responseText);
//        alert(this.responseText);
...
//7
//function deezer_addSongs(ids) {
//    let url = DEEZER_ADD_SONG;
//    url += playlistId + '/tracks';
//    let body = JSON.stringify({ "uris": ids });
//    debugger;
//    deezer_callAPI('POST', url, body, handleDeezerAddRes);
//}

//8
//function handleDeezerAddRes() {
//    if (this.status == 201) { alert("Playlist montada com sucesso.") }
//    else {
//        console.log(this.responseText);
//        alert(this.responseText);
